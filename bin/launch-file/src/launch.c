...
int main()
{
    ...
    // https://stackoverflow.com/questions/43692986/launching-an-application-while-sourcing-bashrc
    ...
    system("bash -i -c \"source /opt/ros/foxy/setup.bash && /opt/app/$(ls | grep 'app-.[^\s]*' | head -1) --no-sandbox\"");
